...
/* ─── Google Fonts ───────────────────────────────────────────────────────── */
if (typeof document !== "undefined" && !document.getElementById("rt-gf")) {
  const l = document.createElement("link");
  l.id = "rt-gf"; l.rel = "stylesheet";
...
const css = `
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
  ...
  /* angle selector — pill toggle group */
  .angle-group{display:flex;gap:8px;flex-wrap:wrap}
  .angle-pill{flex:1;min-width:80px;padding:10px 8px;border-radius:10px;
  ...
  .angle-pill .ap-icon{font-size:20px;display:block}
  .angle-pill.active{border-color:${C.green};background:#EEF5F2;color:${C.green};font-weight:600}
  .angle-pill:hover:not(.active){border-color:#b0aca4;background:#F5F3EF;color:${C.black}}

  .wm-hint{font-size:11px;color:${C.muted};margin-top:8px;line-height:1.5}
  ...
  @media(max-width:600px){
    .wm-card{padding:32px 20px 28px}
    .wm-bc{padding:14px 20px 0}
    .angle-group{gap:6px}
  }
`;
...
function DropZone({ file, onFile, onRemove }) {
  ...
  if (file) {
    return (
      <div className="drop-z" style={{ padding: "18px 16px" }}
...
export default function Rotate() {
  ...
  const handleRotate = async () => {
    if (!file) { setError("Please select a file to rotate."); return; }
    try {
  ...
  const reset = () => { setFile(null); setAngle(90); setResult(null); setError(null); setProc(false); };
            ...
            {!result ? (
              ...
              >
                ...
                <p style={{
                  ...
                  Quickly rotate pages in your PDF document to the correct orientation.
                </p>
                ...
                <div>
                  ...
                  <div className="angle-group">
                    {ANGLES.map(({ value, label, icon }) => (
                      <button
                        key={value}
                        className={`angle-pill${angle === value ? " active" : ""}`}
                  ...
                  <p className="wm-hint">Select the degree to rotate every page in the document.</p>
                </div>
